...
async def next_page(bot, query):
    ...
    if not search:
        await query.answer(f"⚠️ Hey {query.from_user.first_name} You are using this for one of my old message, send the request again ⚠️",show_alert=True)
        return
...
async def advantage_spell_chok(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply(f"Hey {msg.from_user.mention} I cant find any movie in that name.")
    ...
    if not movielist:
        k = await msg.reply(f"Hey {msg.from_user.mention} I cant find anything related to that. Check your spelling")
        await asyncio.sleep(8)
        await k.delete()
